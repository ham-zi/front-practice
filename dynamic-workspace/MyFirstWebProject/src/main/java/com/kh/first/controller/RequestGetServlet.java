...
public class RequestGetServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 */
		 ...
		 /* => 값 뽑기 ( 1단계 )
		 * (경우의 수를 생각해야함) => 셀 수 없음
		 * 정상적인출력 "이다산" / 빈문자열 "" / null
		 * if문으로 널인지 확인하고 추출할것.
		 * */
		//그렇기 떄문에 앞에서 자바스크립트로 검사를 한번 해야함. 
		String name = request.getParameter("name");
		System.out.println(name);
		
		String gender = request.getParameter("gender");
		System.out.println(gender);
		//url에서 변경할 수 있기 때문에 경우가 셀수없음 ->get방식에서는 위험도가 높음
		// 셀수없음 // 선택X null / 
		
		int age = Integer.parseInt(request.getParameter("age"));
		System.out.println(age);
		// "15" // 빈문자열"" : NumberFormatException // 
		...
		// 		=> 경로를 잘못 적었거나 오타 났을 때
		// 500 : 자바 소스코드 상의 오류 (예외발생) -> 내부 서버 오류
		
		// 2단계 : 가공
		//Person person = new Person(name, gender, age, city, height, foods);
		
		// 3단계 : 요청 처리(Service단 호출)
		//Insert라고 가정 : Contorller에서 Service객체를 생성한 후 service단의 메소드를 호출 하면서
		// person객체의 주소 값을 전달 한 뒤 service에서는 dao단을 호출하여 DB ㄴSQL(insert문)을 수행한뒤
		// -> 정수 형태의 결과값을 반환
		// 컨트롤러는 서비스로부터 결과값을 반환받음
		// int result = new PersonService().savePerson(person)
		
		// 4단계 : 결과 값 반환 or 응답화면 지정
		
		// 순수 Servlet만 사용해서 응답 데이터 넘기기
	...
	// 1. 화면을 하루에도 수십번 바꾸는데
	// 서버를 그럴 때 마다 껐다 켜야하는 문제
	
	// 2. 많은 HTML CSS JS를 적기 불편함
